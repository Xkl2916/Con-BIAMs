...
class MULTModel(nn.Module):
    def __init__(self, hyp_params):
        ...
        # 这个是之前创建的那个argparse模块， 用来解析命令行参数的
        self.hp = hp = hyp_params

        # 这些东西是啥？？？ 应该书输出的向量长度 origin， 这里没有搞明白， 这里是main函数定义的一些东西， 我到现在还没有搞懂
        ...
        # 其实这里我不太清楚， 为什么要有这么多的dropout
        self.num_heads = hp.num_heads               # 头的数量
        self.layers = hp.layers                     # 多少层

        self.attn_dropout = hp.attn_dropout         # 丢弃率
        self.attn_dropout_a = hp.attn_dropout_a     # 好像是每一个
        self.attn_dropout_v = hp.attn_dropout_v

        self.relu_dropout = hp.relu_dropout         # 激活函数的丢弃率
        ...
        self.attn_mask = hp.attn_mask               # transformer 中 有自注意力机制 需要挡住， 然后有个mask

        # 1*1 的卷积用于 ResNet50 进行连接
        # configuration for input projection 1D-CNN (optional)
        self.l_ksize = hp.l_ksize
        self.v_ksize = hp.v_ksize
        self.a_ksize = hp.a_ksize

        self.last_bi = True                         # 这个是啥？？？

        combined_dim = 4 * self.d_l   # rigorously 2 * d_l + d_a + d_v      # 全连接的 看论文中图片就能明白

        # 输出的维度
        output_dim = hp.output_dim        # This is actually not a hyperparameter :-)

        # 下面的步骤就是按照一个基本的transformer顺序进行定义的

        # 1. Language embedding layer
        self.embedding = LanguageEmbeddingLayer(self.hp)

        # 2. Temporal convolutional layers  时序卷积
        self.SequenceEncoder = SeqEncoder(self.hp)

        # 3. Shared Encoder (deprceated, inserted)  共享编码器， 这个是干啥的？？？

        # 4. Modal Interaction                      模态之间的相互作用， 分为两个， 分别是文本-音频 和 文本-视觉
        self.modal_interaction = nn.ModuleDict({
            'lv': self.get_network(layers=self.layers),
            'la': self.get_network(layers=self.layers)
        })
        
        # 5. Self Attentions (Could be replaced by LSTMs, GRUs, etc.)           # 自注意力机制 中 可以被这些东西给替换掉
        #    [e.g., self.trans_x_mem = nn.LSTM(self.d_x, self.d_x, 1)
        # self.trans_l_mem = self.get_network(self_type='l_mem', layers=3)
        # self.trans_a_mem = self.get_network(self_type='a_mem', layers=3)
        # self.trans_v_mem = self.get_network(self_type='v_mem', layers=3)

        # 6. Projection layers                                      # 最后的那个投影层， 就是一个全连接层
    ...
    def _forward_last_pooling(self, tensor, lengths, mode='avg'):   # 在最后一层中 取 平均池化 还是最大池化
